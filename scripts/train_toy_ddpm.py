...
from generative_models.training.toy_ddpm import ToyDDPMTrainingConfig, train_toy_ddpm
...
def save_loss_plot(losses: list[float], path: Path) -> Path:
    ...
    ax.set_title("Toy DDPM Training Loss")
    ax.set_xlabel("step")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/toy_ddpm.yaml"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("runs/toy_ddpm"),
    )
    parser.add_argument(
        "--device",
        type=str,
...
def main() -> None:
    ...
    result = train_toy_ddpm(config, device=args.device)
